...
export const StudentDailyGoals = ({ planData, userData }: { planData: any, userData: any }) => {
    ...
    useEffect(() => {
        const fetchSchedule = async () => {
            ...
            const q = query(
                collection(db, 'student_schedule'), 
                where('userId', '==', user.uid),
                where('planId', '==', planData.id) 
            );
            const snap = await getDocs(q);
            setSchedule(snap.docs.map(d => ({ ...d.data(), docId: d.id })));
        };
        fetchSchedule();
    }, [user, planData]);
    ...
    const extractMetaIdsFromItem = (item: any): string[] => {
        const ids: string[] = [];
        if (item.subjects) item.subjects.forEach((s:any) => s.topics?.forEach((t:any) => t.goals?.forEach((g:any) => ids.push(g.id))));
        if (item.topics) item.topics.forEach((t:any) => t.goals?.forEach((g:any) => ids.push(g.id)));
        return ids;
    };
