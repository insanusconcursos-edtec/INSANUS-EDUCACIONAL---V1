...
export const StudentGoalCard: React.FC<StudentGoalCardProps> = ({ goal, onStart, onToggleComplete, onRefresh }) => {
  ...
  const hasContent = videoCount > 0 || fileCount > 0 || linkCount > 0 || hasMindMap || hasFlashcards;
  ...
  const handleMergeExtension = async (e: React.MouseEvent) => {
    ...
    try {
        ...
        // We can pass current date string.
        const today = new Date().toISOString().split('T')[0];
        eventPayload.date = today; 
    ...
    >
      ...
      <div className="p-5 pb-2 flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          {goal.part && goal.part > 0 && !isMerged && (
            <span 
      ...
      {isLessonGoal && hasContent && (
        <div 
            className="p-3 bg-zinc-950/30 border-t border-zinc-800/50 flex justify-center cursor-pointer hover:bg-zinc-900/50 transition-colors"
            onClick={(e) => {
                e.stopPropagation();
                setIsOpen(!isOpen);
            }}
      ...
      {!isLessonGoal && (
        <div className="p-4 mt-auto">
            {hasContent && (
                <div 
                    className="flex items-center justify-center mb-3 cursor-pointer text-zinc-600 hover:text-white transition-colors"
                    onClick={(e) => {
                        e.stopPropagation();
                        setIsOpen(!isOpen);
                    }}
